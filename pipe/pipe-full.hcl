...
intsig M_dstE2 'ex_mem_curr->deste2'	# Destination E2 register ID
intsig M_valE  'ex_mem_curr->vale'      # ALU E value
intsig M_valE2  'ex_mem_curr->vale2'      # ALU2 E value2
...
intsig W_dstE2 'mem_wb_curr->deste2'	# Destination E2 register ID
intsig W_valE  'mem_wb_curr->vale'      # ALU E value
intsig W_valE2  'mem_wb_curr->vale2'      # ALU2 E value2
...
bool instr_valid = f_icode in
	{ INOP, IHALT, IRRMOVL, IIRMOVL, IRMMOVL, IMRMOVL,
	  IOPL, IJXX, ICALL, IRET, IPUSHL, IPOPL, IIOPL, ILEAVE, IENTER, IRMXCHG };
...
int d_srcA = [
	...
	D_icode in { ILEAVE, IENTER } : REBP;
	1 : RNONE; # Don't need register
];

## What register should be used as the B source?
int d_srcB = [
	D_icode in { IRMMOVL, IMRMOVL, IIOPL, IRMXCHG  } : D_rB;
	D_icode == IOPL && D_ifun != ALUMUL && D_ifun != ALUDIV: D_rB;
	D_icode == IOPL && (D_ifun == ALUMUL || D_ifun == ALUDIV): REAX;
	D_icode in { IPUSHL, IPOPL, ICALL, IRET, IENTER } : RESP;
...
int d_dstE = [
	D_icode in { IRRMOVL, IIRMOVL, IIOPL } : D_rB;
	D_icode == IOPL && D_ifun != ALUMUL && D_ifun != ALUDIV: D_rB;
	D_icode == IOPL && (D_ifun == ALUMUL || D_ifun == ALUDIV): REAX;
	D_icode in { IPUSHL, IPOPL, ICALL, IRET, ILEAVE, IENTER } : RESP;
	1 : RNONE;  # Don't write any register
];

int d_dstE2 = [
	D_icode == IOPL && (D_ifun == ALUMUL || D_ifun == ALUDIV): REDX;
	D_icode == IENTER: REBP;
	1 : RNONE;  # Don't write any register
];
...
int d_valA = [
	...
	d_srcA == e_dstE2 : e_valE2;    # Forward valE2 from execute
	d_srcA == M_dstM : m_valM;    # Forward valM from memory
	d_srcA == M_dstE : M_valE;    # Forward valE from memory
	d_srcA == M_dstE2 : M_valE2;    # Forward valE2 from memory
	d_srcA == W_dstM : W_valM;    # Forward valM from write back
	d_srcA == W_dstE : W_valE;    # Forward valE from write back
	d_srcA == W_dstE2 : W_valE2;    # Forward valE2 from write back
	1 : d_rvalA;  # Use value read from register file
];

int d_valB = [
	d_srcB == e_dstE : e_valE;    # Forward valE from execute
	d_srcB == e_dstE2 : e_valE2;    # Forward valE2 from execute
	d_srcB == M_dstM : m_valM;    # Forward valM from memory
	d_srcB == M_dstE : M_valE;    # Forward valE from memory
	d_srcB == M_dstE2 : M_valE2;    # Forward valE2 from memory
	d_srcB == W_dstM : W_valM;    # Forward valM from write back
	d_srcB == W_dstE : W_valE;    # Forward valE from write back
	d_srcB == W_dstE2 : W_valE2;    # Forward valE2 from write back
...
int aluA = [
	...
	E_icode in { ICALL, IPUSHL, IENTER } : -4;
...
int aluB = [
	E_icode in { IRMMOVL, IMRMOVL, IOPL, ICALL,
		     IPUSHL, IRET, IPOPL, IIOPL, ILEAVE, IENTER, IRMXCHG } : E_valB;
...
int mem_addr = [
	M_icode in { IRMMOVL, IPUSHL, ICALL, IMRMOVL, IENTER, IRMXCHG } : M_valE;
...
bool mem_write = M_icode in { IRMMOVL, IPUSHL, ICALL, IENTER, IRMXCHG };
